...
def inferir_e_converter_tipos(df, colunas_texto=None, colunas_moeda=None):
    """
    Converte colunas de moeda para float, colunas de texto para string,
    e tenta inferir datas nas demais colunas de objeto.
    """
    df_copy = df.copy()
...
def encontrar_colunas_tipos(df):
    """
    Retorna as colunas numéricas e de data baseadas nos tipos de dados do DataFrame.
    """
...
def verificar_ausentes(df, colunas):
    """
    Retorna um dicionário com o número de valores ausentes por coluna.
    """
    resultado = {}
    for col in colunas:
