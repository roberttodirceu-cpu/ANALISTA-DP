# app.py - Versão FINAL e COMPLETA com BOTÕES DE DATASET
...
with st.sidebar:
    ...
    # Seção 1: Trocar Dataset Ativo (AGORA COM BOTÕES INDIVIDUAIS)
    if st.session_state.data_sets_catalog:
        st.header("1. Trocar Dataset Ativo")
        dataset_names = list(st.session_state.data_sets_catalog.keys())
        
        # ⚠️ MUDANÇA AQUI: Itera e cria um botão para cada dataset salvo
        for name in dataset_names:
            
            is_active = (name == st.session_state.current_dataset_name)
            
            # Use 'primary' se for o dataset ativo, senão use 'secondary' ou 'tertiary'
            button_type = 'primary' if is_active else 'secondary'
            
            # Cria o rótulo do botão no formato que você deseja [NOME_ARQUIVO]
            button_label = f"[{name}]"
            
            if st.button(
                button_label, 
                key=f'switch_btn_{name}', 
                use_container_width=True,
                type=button_type
            ):
                # Ação: Trocar o dataset ativo
                switch_dataset(name)
